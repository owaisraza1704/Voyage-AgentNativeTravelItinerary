...
function isAffirmativeReply(value: string) {
  return /^(yes|yeah|yep|yup|confirm|confirmed|go ahead|do it|proceed|please do)([.!?,\s]|$)/i.test(
    value.trim(),
  )
}
...
export function SiteShell({
  ...
  return (
    <div className="min-h-screen bg-ivory text-charcoal font-sans">
      ...
      <main>{children}</main>
      <VoyageCommandDock open={agentOpen} onClose={() => setAgentOpen(false)} />
    </div>
  )
}
...
function ActiveBookingBanner() {
  return (
    <div className="fixed left-1/2 top-20 z-30 flex -translate-x-1/2 items-center gap-4 border border-gold/40 bg-ivory px-4 py-2 text-xs text-charcoal shadow-md">
      <span>
        <span className="mr-1 uppercase tracking-[0.14em] text-gold">
          Active booking
        </span>{" "}
        You already have a confirmed itinerary.
      </span>
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 border-b transition-colors ${
        isHome
          ? "border-white/15 bg-charcoal/20 text-white backdrop-blur-sm"
          : "border-sand bg-ivory/95 text-charcoal backdrop-blur-md"
      }`}
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-6 px-6">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={onOpenAgent}
            className={`flex items-center gap-2 px-4 py-2 text-[13px] tracking-wide transition-colors ${
              isHome
                ? "border border-white/35 hover:bg-white/10"
      ...
      {menuOpen && (
        <div
          className={`border-t px-6 py-4 md:hidden ${
            isHome ? "border-white/15 bg-charcoal/95" : "border-sand bg-ivory"
          }`}
        >
...
function VoyageCommandDock({
  ...
  async function submitMessage(value: string) {
    ...
    if (isNegativeReply(trimmed) && (bookingSummary || cancellationSummary)) {
      ...
      setConversation((current) => [
        ...
        {
          role: "assistant",
          content: "Understood. I won't change your booking.",
        },
      ])
      return
    }
    ...
    try {
      const tools =
        availableTools.length > 0 ? availableTools : await getWebMcpTools()
        ...
        if (!assistantMessage.tool_calls?.length) {
          setConversation((current) => [
            ...current,
            {
              role: "assistant",
              content:
                assistantMessage.content ?? "I could not produce a response.",
            },
          ])
          return
        }
        ...
        for (const toolCall of assistantMessage.tool_calls) {
          ...
          if (toolName === "book_itinerary" || toolName === "cancel_booking") {
            ...
            toolResult = {
              ...
              message:
                "Show the user the relevant summary and wait for an explicit confirmation in the UI before executing this action.",
            }
          } else {
            ...
            if (!tool) {
              ...
              try {
                ...
                if (toolName === "get_booking_summary") {
                  const summaryResult = toolResult as {
                    ok?: boolean
            ...
            setConversation((current) => [
              ...current,
              {
                ...
                content:
                  ...
                  "You already have an active booking. Cancel it from My Trips before planning another itinerary.",
              },
            ])
            return
          }
      ...
      throw new Error(
        `The agent reached its ${maxAgentTurns}-step tool-call limit.`,
      )
    } catch (error: unknown) {
      setConversation((current) => [
        ...current,
  ...
  return (
    <div className="fixed left-1/2 top-[4.75rem] z-50 w-[calc(100%-1.5rem)] max-w-3xl -translate-x-1/2" role="dialog" aria-label="Voyage command dock">
      <section className="flex max-h-[calc(100dvh-5.5rem)] flex-col overflow-hidden border border-sand bg-ivory/95 shadow-xl backdrop-blur-md">
        <div className="flex items-center gap-3 px-3 py-3 sm:gap-4 sm:px-5">
          ...
          <button onClick={clearChat} disabled={isThinking} aria-label="Clear chat" className="hidden text-[10px] uppercase tracking-[0.14em] text-taupe transition-colors hover:text-charcoal disabled:opacity-50 sm:block">Clear</button>
          <button onClick={onClose} className="p-1.5 text-taupe transition-colors hover:text-charcoal" aria-label="Close Voyage dock"><CloseIcon className="h-4 w-4" /></button>
        </div>
        ...
        {detailsOpen && <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain border-t border-sand bg-mist px-4 py-4 sm:px-5">
          <div className="mb-4 flex items-center justify-between text-[10px] uppercase tracking-[0.16em]">
            <span className="text-taupe">Live conversation</span>
            <span className={toolStatus === "connected" ? "text-green-800" : "text-taupe"}>WebMCP · {toolStatusLabel}</span>
          </div>
...
function BookingConfirmationCard({
  ...
  return (
    <section
      className="border border-gold/60 bg-gold/5 p-5"
    ...
    >
      ...
      <h3 className="mt-2 font-serif text-2xl text-charcoal">
        {summary.selectedStay.name}
      </h3>
      <p className="mt-1 text-sm text-taupe">
        {summary.destination.name}, {summary.destination.country}
      </p>
      <div className="mt-5 space-y-2 border-t border-gold/20 pt-4 text-sm">
        <div className="flex justify-between gap-4">
          ...
          <span className="text-right text-charcoal">
            {formatDateRange(summary.dates.startDate, summary.dates.endDate)}
          </span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-taupe">Travelers</span>
          <span className="text-right text-charcoal">
            {summary.travelers.adults} adults
            {summary.travelers.children
              ? `, ${summary.travelers.children} children`
              : ""}
          </span>
        </div>
        <div className="flex justify-between gap-4">
          ...
          <span className="text-right text-charcoal">
            {summary.dates.nights} nights
          </span>
        </div>
        <div className="flex justify-between gap-4 border-t border-gold/20 pt-3">
      ...
      {bookingError && (
        <p className="mt-4 border border-red-900/20 bg-red-50/50 p-3 text-xs leading-relaxed text-red-900">
          {bookingError}
        </p>
      )}
      <div className="mt-5 flex gap-3">
        <button
          onClick={onConfirm}
          disabled={disabled}
          className="flex-1 bg-charcoal px-4 py-3 text-xs uppercase tracking-[0.12em] text-ivory transition-colors hover:bg-gold disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={disabled}
          className="border border-sand px-4 py-3 text-xs text-charcoal transition-colors hover:border-charcoal disabled:cursor-not-allowed disabled:opacity-50"
        >
          Keep planning
        </button>
      </div>
...
function CancellationConfirmationCard({
  booking,
  bookingError,
  disabled,
  onConfirm,
  ...
  return (
    <section
      className="mt-6 border border-red-900/30 bg-red-50/30 p-5"
      aria-label="Cancellation confirmation"
    >
      <p className="text-[10px] uppercase tracking-[0.18em] text-red-900">
      ...
      <p className="mt-1 text-sm text-taupe">
        {booking.destinationName}, {booking.destinationCountry}
      </p>
      <div className="mt-5 space-y-2 border-t border-red-900/15 pt-4 text-sm">
        <div className="flex justify-between gap-4">
          <span className="text-taupe">Confirmation</span>
          <span className="text-right text-charcoal">{booking.reference}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-taupe">Dates</span>
          <span className="text-right text-charcoal">
      ...
      <div className="mt-5 flex gap-3">
        <button
          onClick={onConfirm}
          disabled={disabled}
          className="flex-1 bg-red-900 px-4 py-3 text-xs uppercase tracking-[0.12em] text-ivory transition-colors hover:bg-charcoal disabled:cursor-not-allowed disabled:opacity-50"
        >
          {disabled ? "Cancelling…" : "Confirm cancellation"}
        </button>
        <button
          onClick={onKeepPlanning}
          disabled={disabled}
