...
type TripAction =
  ...
  | { type: "hotel/remove" }
  | { type: "bookings/load"; activeBooking: BookingRecord | null; bookings: BookingRecord[] }
  ...
  switch (action.type) {
    ...
    case "bookings/load":
      return {
        ...
        destinationId: action.activeBooking?.destinationId ?? state.destinationId,
    ...
    case "booking/confirmed":
      return { ...state, selectedHotelId: action.booking.hotelId, destinationId: action.booking.destinationId, startDate: action.booking.startDate, endDate: action.booking.endDate, adults: action.booking.adults, children: action.booking.children, bookingStatus: "confirmed", bookingReference: action.booking.reference, bookedItinerary: action.booking, bookingHistory: [action.booking, ...state.bookingHistory.filter((booking) => booking.reference !== action.booking.reference)] }
    case "booking/cancelled":
      return { ...state, selectedHotelId: null, bookingStatus: "draft", bookingReference: null, bookedItinerary: null, bookingHistory: [action.booking, ...state.bookingHistory.filter((booking) => booking.reference !== action.booking.reference)] }
  }
}
...
export function VoyageProvider({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => {
    ...
    fetch("/api/bookings")
      ...
      .then((payload: { activeBooking: BookingRecord | null; bookings: BookingRecord[] }) => {
        if (!disposed) dispatch({ type: "bookings/load", activeBooking: payload.activeBooking, bookings: payload.bookings })
      })
      .catch((error: unknown) => {
        if (!disposed) setBookingError(error instanceof Error ? error.message : "Unable to load bookings.")
      })
      .finally(() => {
    ...
    () => ({
      ...
      confirmBooking: async () => {
        ...
        const response = await fetch("/api/bookings", {
          ...
          body: JSON.stringify({ hotelId: state.selectedHotelId, destinationId: state.destinationId, startDate: state.startDate, endDate: state.endDate, adults: state.adults, children: state.children }),
        })
        ...
        dispatch({ type: "booking/confirmed", booking: payload as BookingRecord })
        return true
      },
      cancelBooking: async () => {
        if (!state.bookedItinerary) return false
        setBookingError(null)
        const response = await fetch(`/api/bookings/${state.bookedItinerary.reference}/cancel`, { method: "POST" })
        const payload = await response.json()
        if (!response.ok) {
          setBookingError(payload.message ?? "Unable to cancel booking.")
          return false
        }
        dispatch({ type: "booking/cancelled", booking: payload as BookingRecord })
        return true
      },
    }),
    [bookingError, bookingsLoading, destination, nights, selectedHotel, state, stayTotal, taxes],
  )
